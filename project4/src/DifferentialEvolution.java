...
public class DifferentialEvolution {
    public Chromosome mutation(Chromosome[] population, Chromosome targetChromosome, int targetChromosomeIndex){
        ...
        for(int i = 0; i<chromosome2.weights.length; i++){
            double difference = chromosome2.weights[i] - chromosome3.weights[i];
            betaMultiplierWeights[i] = difference*beta;
        }

        double[] trialChromosomeWeights = new double[]{};

        for(int i = 0; i<chromosome1.weights.length; i++){
            double sum = chromosome1.weights[i] + betaMultiplierWeights[i];
            trialChromosomeWeights[i] = sum;
        }
        trialChromosome.weights = trialChromosomeWeights;
